...
public class WriteLogFilesTemp {
  ...
  public static void main(String[] args) throws IOException {
    ...
    date = new Date();
    aFile = new File("DataTemp" + dateFormat.format(date) + ".csv");
    if (aFile.exists()){
       aFile.renameTo(new File("DataTemp-" + dateFormat.format(date) + "-1.csv"));
    ...
    fout.write("Time,Temp,Differential Temp\n");
    fout.close();
  ...
  public static void Writetofile(String x) throws IOException{
        ...
        File f = null;
            try {
                inputFile = new File("DataTemp" + dateFormat.format(date) + ".csv");
                f = inputFile;
                outputFile = new File("DataTempTemp.csv");
                try {
