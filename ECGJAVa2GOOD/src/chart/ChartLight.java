 ...
 *
 */
public final class ChartLight
    extends JApplet implements Runnable{
    static public boolean stop = false;

    public void run() {
            JFrame frame = new JFrame("Light Data Chart");
            frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
            ChartLight showcase = new ChartLight();
            showcase.init();
  ...
  final class ControlPanel
    ...
    private void createAmountPointSlider() {
      ...
      int maxPoints = ChartLight.this.getTrace().getMaxSize();
      ...
      this.m_amountPointsSlider.addChangeListener(new ChangeListener() {
        public void stateChanged(final ChangeEvent e) {
          ...
          if (!source.getValueIsAdjusting()) {
            ...
            ChartLight.this.getTrace().setMaxSize(value);
          }
        }
      });
    ...
    private void createColorChooserButton() {
      ...
      this.m_colorChooser.addActionListener(new ActionListener() {
        public void actionPerformed(final ActionEvent ae) {
          ...
          ChartLight.this.getTrace().setColor(color);
        }
      });
    ...
    private void createLatencySlider() {
      ...
      this.m_latencyTimeSlider.setValue((int) ChartLight.this.getCollector().getLatency());
      ...
      this.m_latencyTimeSlider.addChangeListener(new ChangeListener() {
        public void stateChanged(final ChangeEvent e) {
          ...
          if (!source.getValueIsAdjusting()) {
            int value = source.getValue();
            ChartLight.this.getCollector().setLatency(value);
          }
        }
      });
    }
    ...
    private void createSnapShotButton() {
      ...
      this.m_snapshot = new JButton(Chart2DActionSaveImageSingleton.getInstance(
          ChartLight.this.m_chart, "Save image"));
    ...
    private void createStartStopButton() {
      ...
      this.m_startStop.addActionListener(new ActionListener() {
        public void actionPerformed(final ActionEvent e) {
          ...
          if (ChartLight.this.getCollector().isRunning()) {
            ChartLight.this.stopData();
            source.setText("start");
            running = false;
          } else {
            ChartLight.this.startData();
            source.setText("stop");
            running = true;
          }
          source.invalidate();
  ...
  public void init() {
    ...
    this.getTrace().setName("Light");
    this.getTrace().setPhysicalUnits("Milliseconds", "Light value");
    ...
    this.setCollector(new DataAcquisitionLight(this.getTrace(), 100));
   ...
   */
  private void setCollector(final DataAcquisitionLight collector) {
