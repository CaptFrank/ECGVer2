 ...
 */
/*
 *  ChartLight.java, the main demo application of project jchart2d.
 ...
 *
 */
public final class ChartECGembed
    extends JApplet implements Runnable{
    static public boolean stop = false;

    public void run() {
            //JFrame frame = new JFrame("ECG Data Chart");
            //frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
            JInternalFrame frame = ecgjava2.ECGJAVa2View.ECGContainer;
            ChartECGembed showcase = new ChartECGembed();
            showcase.init();
  ...
  final class ControlPanel
    ...
    protected ControlPanel() {
      ...
      stretch.add(Box.createHorizontalGlue());
      this.add(stretch);
      ecgjava2.ECGJAVa2View.ECGContainer.repaint();
      this.repaint();
    }
    ...
    private void createAmountPointSlider() {
      ...
      int maxPoints = ChartECGembed.this.getTrace().getMaxSize();
      ...
      this.m_amountPointsSlider.addChangeListener(new ChangeListener() {
        public void stateChanged(final ChangeEvent e) {
          ...
          if (!source.getValueIsAdjusting()) {
            ...
            ChartECGembed.this.getTrace().setMaxSize(value);
          }
        }
      });
    ...
    private void createColorChooserButton() {
      ...
      this.m_colorChooser.addActionListener(new ActionListener() {
        public void actionPerformed(final ActionEvent ae) {
          ...
          ChartECGembed.this.getTrace().setColor(color);
        }
      });
    ...
    private void createLatencySlider() {
      ...
      this.m_latencyTimeSlider.setValue((int) ChartECGembed.this.getCollector().getLatency());
      ...
      this.m_latencyTimeSlider.addChangeListener(new ChangeListener() {
        public void stateChanged(final ChangeEvent e) {
          ...
          if (!source.getValueIsAdjusting()) {
            int value = source.getValue();
            ChartECGembed.this.getCollector().setLatency(value);
          }
        }
      });
    }
    ...
    private void createSnapShotButton() {
      ...
      this.m_snapshot = new JButton(Chart2DActionSaveImageSingleton.getInstance(
          ChartECGembed.this.m_chart, "Save image"));
    ...
    private void createStartStopButton() {
      ...
      this.m_startStop.addActionListener(new ActionListener() {
        public void actionPerformed(final ActionEvent e) {
          ...
          if (ChartECGembed.this.getCollector().isRunning()) {
            ChartECGembed.this.stopData();
            source.setText("start");
            running = false;
          } else {
            ChartECGembed.this.startData();
            source.setText("stop");
            running = true;
          }
          source.invalidate();
    ...
    public void createStopButton(){
         // Start stop Button
      this.stopData = new JButton("Stop Data");
      this.stopData.setBackground(Color.WHITE);
      this.stopData.addActionListener(new ActionListener() {
        public void actionPerformed(final ActionEvent e) {
          JButton source = (JButton) e.getSource();
          ChartECGembed.stop = true;
          source.invalidate();
          source.repaint();
        }
      });
  ...
  public void init() {
    ...
    this.setChart(chart);
    this.setSize(new Dimension(525, 265));
    ...
    this.getTrace().setName("Live ECG");
    this.getTrace().setPhysicalUnits("Milliseconds", "ECG Value value");
    ...
    this.setCollector(new DataAcquisitionECG(this.getTrace(), 100));
   ...
   */
  private void setCollector(final DataAcquisitionECG collector) {
